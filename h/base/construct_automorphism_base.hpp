...
namespace base {
	...
	void writeautom(T *p, T n, std::vector<std::vector<T>>& automorphism_base, SIZE & num_tot) {
	    ...
	    for (T i = 0; i < n; ++i) permutation[i] = p[i];
	...
	template <typename T,typename SIZE>
	static void my_groupelts(levelrec *lr, int n, int level,int *before, int *after, int *id,std::vector<std::vector<T>>& automorphism_base, SIZE & num_tot) {
	    int i,j,orbsize;
	    int *p,*cr;
	    cosetrec *coset;
	    ...
	    for (j = 0; j < orbsize; ++j) {
	        ...
	        if (level == 0) writeautom((p == NULL ? id : p),n,automorphism_base,num_tot);
	        else my_groupelts(lr,n,level-1,p,after+n,id,automorphism_base,num_tot);
	    }
	}

	// Call writeautom for every element of the group, including the identity.
	template <typename T,typename SIZE>
	void my_allgroup(grouprec *grp, std::vector<std::vector<T>>& automorphism_base, SIZE & num_tot) {
	    int i,depth,n;
	    ...
	    if (depth == 0) {
	        writeautom(id,n,automorphism_base,num_tot);
	        return;
	    }

	    DYNALLOC1(int,allp,allp_sz,n*depth,"malloc");
	    my_groupelts(grp->levelinfo,n,depth-1,NULL,allp,id,automorphism_base,num_tot);
	}

	template <typename T,typename SIZE,typename SIZE_TP>
	SIZE construct_automorphism_base(T ** S,const SIZE num_rows,const SIZE num_cols, const SIZE_TP n, const SIZE_TP m, std::vector<std::vector<T>>& automorphism_base) {
	    ...
	    SIZE num_autom_base = 0;
	    my_allgroup(group,automorphism_base,num_autom_base);
