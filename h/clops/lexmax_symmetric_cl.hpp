...
namespace clops {
	...
	bool is_outside_X(T ** orbits_i,T ** ground_H, T * A_sym, T * A_indices,const SIZE num_A_indices,const SIZE length_A,const T D) {
		std::memset(A_sym+1,0,(length_A-1) * sizeof(T));
		for (SIZE j = 1; j < num_A_indices; ++j) {
		    if (orbits_i[A_indices[j]][0] == 0) return true;
		    else A_sym[array::index_in_collection(ground_H,orbits_i[A_indices[j]],length_A,D)] = 1;
		}
		return false;
	}
	...
	template <typename T,typename SIZE>
	void lexmax_symmetric_cl(T * A,T *& symcl,T ** ground_H, const SIZE length_A,T *** orbits,const SIZE num_autom_base,const T D) {
	    SIZE i;
	    ...
	    if (!array::is_all_ones(A,length_A)) {
	        ...
	        if (!array::is_all_zeros(A + 1,length_A - 1)) {
	            for (i = 0; i < num_autom_base; ++i) {
	                if (!clops::is_outside_X(orbits[i],ground_H,A_sym,A_indices,num_A_indices,length_A,D))
